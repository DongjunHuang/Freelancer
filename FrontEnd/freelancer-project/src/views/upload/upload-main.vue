...
function updateState(next: UploadStatePatch) {
  ...
  if (next.config) Object.assign(uploadState.config, next.config)
...
<template>
  <div ref="uploadStatusEl" class="max-w-xl mx-auto">
    ...
    <div class="pt-4">
      <UploadFilePane :state="uploadState" @update:state="updateState" />
    </div>

    <!--Select required upload file config info-->
    <div class="pt-4" v-show="uploadState.dataset.isNew">
      <UploadFileConfigPane :state="uploadState" @update:state="updateState" />
    </div>
    ...
    <div class="pt-4 flex items-center gap-3 relative">
      ...
      <span
        ...
        class="flex items-center justify-center h-5 min-w-[20px] px-1 rounded-full text-[11px] font-semibold transition-all duration-300"
        :class="{
      ...
      >
        <template v-if="progress < 100 && !failed"> {{ progress }}% </template>
        <template v-else-if="progress === 100 && !failed"> ✓ </template>
