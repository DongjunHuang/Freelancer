...
async function onResendEmailClick() {
  ...
  try {
    ...
    message.error(e?.response?.data?.message || e?.message || 'Failed to resend email.')
  } finally {
...
<template>
    ...
    <main class="flex-1">
      <div class="max-w-md mx-auto px-4 pt-8">
        ...
        <form class="space-y-4" @submit.prevent="submit">
          <input
            v-model.trim="f.signin.username"
            type="text"
  ...
  >
    <div class="space-y-2">
      <p>Your account is registered but the email address has not been verified yet.</p>
      <p>
        Please check your inbox (and spam folder). If you didn't receive the email, you can resend
    ...
    <template #action>
      <n-space justify="end">
        <n-button quaternary @click="onCloseVerifyModal"> Cancel </n-button>
        <n-button type="primary" :disabled="resendCooldown > 0" @click="onResendEmailClick">
          <template v-if="resendCooldown > 0"> Resend in {{ resendCooldown }} </template>
