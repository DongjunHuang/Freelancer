...
async function submit() {
  if (!isSignupFormValid.value || loading.value) return
  error.value = ''
  success.value = ''
  loading.value = true
  ...
  try {
    const res = await signup({
      username: f.signup.username,
      password: f.signup.password,
      email: f.signup.email,
    })

    success.value =
      res.data?.message || 'Registration successful. Please check your email the link to verify.'
    ...
    <main class="flex-1">
      <div class="max-w-md mx-auto px-4 pt-8">
        ...
        <form class="space-y-4" @submit.prevent="submit">
          <input
            v-model.trim="f.signup.username"
            type="text"
