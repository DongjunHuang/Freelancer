...
<template>
  <div class="mt-4 rounded-2xl bg-white p-4 shadow-sm">
    <h3 class="text-sm font-semibold text-slate-900">Date & Symbol settings</h3>
    ...
    <div v-if="props.state.config.headers.length" class="mt-3 space-y-3">
      <!-- Date column -->
      <div>
        <span class="block text-xs font-medium text-slate-700"> Date column </span>
        <select
        ...
        >
          ...
          <option v-for="h in props.state.config.headers" :key="h" :value="h">
            {{ h }}
          </option>
        </select>
      </div>
      ...
      <!-- Date format -->
      <div>
        <span class="block text-xs font-medium text-slate-700"> Date format </span>
        <select
      ...
      <!-- symbol column -->
      <div>
        <span class="block text-xs font-medium text-slate-700"> Symbol column </span>
        <select
          v-model="symbol"
          class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 outline-none focus:border-slate-400 focus:bg-white"
        >
          <option disabled value="">-- Select --</option>
          <option v-for="h in props.state.config.headers" :key="h + '-symbol'" :value="h">
            {{ h }}
          </option>
        </select>
        <p class="mt-1 text-[11px] text-slate-400">
